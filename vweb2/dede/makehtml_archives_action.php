...
{
    $dsql->Execute('out',"SELECT id FROM `#@__arctiny` $idsql ORDER BY typeid ASC,id ASC $limitSql");
} else {
    $dsql->Execute('out',"SELECT id FROM `#@__arctiny` $idsql ORDER BY id ASC $limitSql");
}
