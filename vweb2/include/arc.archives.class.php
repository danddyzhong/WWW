...
class Archives
{
    ...
    function __construct($aid)
    {
        ...
        $this->dsql = $dsql;
        $query = "SELECT channel,typeid FROM `#@__archives` WHERE id='$aid' ";
    ...
    {      
        ...
        //记录创建的文件名
        file_put_contents(DEDEROOT."/a/v_play/pageName.html",  $filename);
    ...
    function GetPreNext($gtype='')
    {
        ...
        if(count($this->PreNext)<2)
        {
            ...
            if(is_array($preRow))
            {
				if($preRow['siteurl']!='') $domainlink='';
                $mlink = $domainlink.GetFileUrl($preRow['id'],$preRow['typeid'],$preRow['senddate'],$preRow['title'],$preRow['ismake'],$preRow['arcrank'],
            ...
            if(is_array($nextRow))
            {
				if($nextRow['siteurl']!='') $domainlink='';
                $mlink = $domainlink.GetFileUrl($nextRow['id'],$nextRow['typeid'],$nextRow['senddate'],$nextRow['title'],$nextRow['ismake'],$nextRow['arcrank'],
