...
class TypeLink
{
    ...
    function GetPositionLink($islink=true)
    {
		 if ( defined('DEDEMOB') )
        {
            $indexpage = "<a href='".$GLOBALS['cfg_mip']."'>".$this->indexName."</a>";
        } else{
    ...
    function GetOneTypeLink($typeinfos)
    {
		...
		if(defined('DEDEMOB')) $domainlink=$GLOBALS['cfg_mip'];
		elseif($typeinfos['siteurl']!='') $domainlink='';
		else $domainlink=$GLOBALS['cfg_basehost'];
		$typelink = "<a href='".$domainlink.$typepage."'>".$typeinfos['typename']."</a>";
        return $typelink;
    }
    ...
    function GetOptionArray($hid=0,$oper=0,$channeltype=0,$usersg=0,$showid=0)
    {
        return $this->GetOptionList($hid,$oper,$channeltype,$usersg,$showid);
    }

    function GetOptionList($hid=0,$oper=0,$channeltype=0,$usersg=0,$showid=0)
    {
        ...
        if($hid>0)
        {
			...
			//
			$showtypeid='';
        	if($showid==1) $showtypeid=' ID:'.$row['id'];
			//
            if($row['ispart']==1) {
                $this->OptionArrayList .= "<option value='".$row['id']."' style='background-color:#DFDFDB;color:#888888' selected>".$row['typename'].$showtypeid."</option>\r\n";
            }
            else {
                $this->OptionArrayList .= "<option value='".$row['id']."' selected>".$row['typename'].$showtypeid."</option>\r\n";
            }
        }
        ...
        while($row=$this->dsql->GetObject())
        {
            if($row->id!=$hid)
            {
				...
				//
                if($row->ispart==1) {
                    $this->OptionArrayList .= "<option value='".$row->id."' style='background-color:#EFEFEF;color:#666666'>".$row->typename.$showtypeid."</option>\r\n";
                }
                else {
                    $this->OptionArrayList .= "<option value='".$row->id."'>".$row->typename.$showtypeid."</option>\r\n";
                }
            }
            $this->LogicGetOptionArray($row->id, "─", $oper, $showid);
        }
     ...
     * @return    string
     */
    function LogicGetOptionArray($id, $step, $oper=0, $showid=0)
    {
        global $cfg_admin_channel;
        if(empty($cfg_admin_channel)) $cfg_admin_channel = 'all';
        ...
        while($row=$this->dsql->GetObject($id))
        {
			...
			//
            if($row->ispart==1) {
                $this->OptionArrayList .= "<option value='".$row->id."' style='background-color:#EFEFEF;color:#666666'>$step".$row->typename.$showtypeid."</option>\r\n";
            }
            else {
                $this->OptionArrayList .= "<option value='".$row->id."'>$step".$row->typename.$showtypeid."</option>\r\n";
            }
            $this->LogicGetOptionArray($row->id, $step."─", $oper, $showid);
        }
    }
