...
class ListView
{
    ...
    function CountRecord()
    {
        ...
        {
			$sql = "SELECT id FROM `#@__arctype` WHERE reid='{$this->TypeID}' And ishidden<>1 order by sortrank asc limit 1 ";
    		$row = $this->dsql->GetOne($sql);   
			if(is_array($row)) $tempfile = $GLOBALS['cfg_basedir'].$GLOBALS['cfg_templets_dir']."/".$GLOBALS['cfg_df_style']."/list_default.htm";
			else $tempfile = $GLOBALS['cfg_basedir'].$GLOBALS['cfg_templets_dir']."/".$GLOBALS['cfg_df_style']."/list_defaultlb.htm";
            if ( defined('DEDEMOB') )
            {
                $tempfile =str_replace('.htm','_m.htm',$tempfile);
            }
    ...
    function GetPageListST($list_len,$listitem="index,end,pre,next,pageno")
    {
		...
		if(defined('DEDEMOB')){
			...
			$po=strripos($file_url,"/");
			$dir_url=substr($file_url,0,$po+1);
			$http_url='http://'.$_SERVER['HTTP_HOST'].$dir_url;
		}
		//
        $tnamerule = $this->GetMakeFileRule($this->Fields['id'],"list",$this->Fields['typedir'],$this->Fields['defaultname'],$this->Fields['namerule2']);
        $tnamerule = $http_url.preg_replace("/^(.*)\//", '', $tnamerule);
        ...
        for($j;$j<=$total_list;$j++)
        {
            if($j==$this->PageNo)
            {
                $listdd.= "<li class=\"thisclass\"><span>$j</span></li>\r\n";
            }
            else
            {
