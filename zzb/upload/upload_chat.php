...
$attachDir='upfile_chat';//上传文件保存路径，结尾不要带/
$dirType=1;//1:按天存入目录 2:按月存入目录 3:按扩展名存目录  建议使用按天存
$maxAttachSize=2097152;//最大上传大小，默认是2M
$upExt='txt,rar,zip,jpg,gif,png,swf,wmv,avi,wma,mp3,mid,ico';//上传扩展名
...
if($err==''){
	...
	if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
	{
		...
		else
		{
			...
			//手机上传头像
			if(isset($_GET['uimg'])&&$_SESSION['login_uid']!=""){
				$targetPath="../face/p1/".$_SESSION['login_uid'].".gif";
			}
			...
			else $msg="{'url':'".$targetPath."','localname':'".jsonString($localName)."','info':'{$_POST[info]}'}";//id参数固定不变，仅供演示，实际项目中可以是数据库ID
		}
	}
